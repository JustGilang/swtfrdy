					...
					<div class="col-md-12 order-details">
						...
						<div class="order-summary">
							...
							<div class="order-products">
								<?php
								$id = $_GET['id'];
								$sql = mysqli_query($conn, "SELECT * FROM `order` WHERE id_member = '$id' AND stats='belum terbayar'");
								while($data = mysqli_fetch_array($sql)){
									$produk = mysqli_query($conn, "SELECT * FROM produk WHERE id_produk='$data[id_produk]'");
									$ambil = mysqli_fetch_array($produk);
								?>
								<div class="order-col">
                                    <div><?= $produk['nama']?></div>
									<div><?= $data['total_harga']?></div>
								</div>
								<?php }?>
							</div>
							<div class="order-col">
								<div>Ongkir</div>
								<div><strong>Rp.30.000</strong></div>
							</div>
							<div class="order-col">
								<div><strong>TOTAL</strong></div>
								<div><strong class="order-total">Rp.230.000</strong></div>
							</div>
						</div>
						<div class="payment-method">
							<div class="input-radio">
								...
								<div class="caption">
									<p>No.Rek 123456789 a/n Sweetfriday</p>
								</div>
							</div>
							<div class="input-radio">
